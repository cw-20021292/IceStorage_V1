...
void InitComm(void)
{
    ...
    /// MAIN
    comm[COMM_ID_MAIN].recvBuf = &mainRecvBuf[0];
    comm[COMM_ID_MAIN].sendBuf = &mainSendBuf[0];
    comm[COMM_ID_MAIN].rxErrLimitTimer = COMM_RX_ERR_LIMIT_TIMEOUT;

    /// MAIN
    comm[COMM_ID_SUB_FRONT].recvBuf = &mainRecvBuf[0];
    comm[COMM_ID_SUB_FRONT].sendBuf = &mainSendBuf[0];
    comm[COMM_ID_SUB_FRONT].rxErrLimitTimer = COMM_RX_ERR_LIMIT_TIMEOUT;
...
void SetMainRxErr(U8 err)
{
    if( err == TRUE )
    {
        if( comm[COMM_ID_MAIN].rxErrLimitTimer == 0U )
        {
            return;
        }
    }

    comm[COMM_ID_MAIN].rxErr = err;
}
...
void SetSubFrontRxErr(U8 err)
{
    if( err == TRUE )
    {
        if( comm[COMM_ID_SUB_FRONT].rxErrLimitTimer == 0U )
        {
            return;
        }
    }

    comm[COMM_ID_SUB_FRONT].rxErr = err;
}


U8 GetMainRxErr(void)
{
    return comm[COMM_ID_MAIN].rxErr;
}

U8 GetSubFrontRxErr(void)
{
    return comm[COMM_ID_SUB_FRONT].rxErr;
}
...
void UpdateMainRxErrTimer(void)
{
    if( comm[COMM_ID_MAIN].rxErrLimitTimer != 0U )
    {
        comm[COMM_ID_MAIN].rxErrLimitTimer--;
    }
}
...
void UpdateSubFrontRxErrTimer(void)
{
    if( comm[COMM_ID_SUB_FRONT].rxErrLimitTimer != 0U )
    {
        comm[COMM_ID_SUB_FRONT].rxErrLimitTimer--;
    }
}

/**
 * @brief Main 통신에러 검사 및 표시
 * 
 */
