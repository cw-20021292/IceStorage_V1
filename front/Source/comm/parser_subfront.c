...
static I16 ParserReqSubFront(U8 *buf);
...
{
    { PKT_REQ_SUB_FRONT, ParserReqSubFront },
};
...
static I16 ParserReqSubFront(U8 *buf)
{
    ...
    // Send ACK 
    SetCommHeader(COMM_ID_SUB_FRONT, PKT_ACK_SUB_FRONT);
    StartTimer(TIMER_USER, TIMER_USER_ID_COMM_FRONT_SUB_TX, 1UL);
    return TRUE;
}
...
static I16 MakePktAckSubFront(U8 *buf);
const static SMakeList_T makeList[] = 
{
    { PKT_ACK_SUB_FRONT,       MakePktAckSubFront },
};
#define SZ_MAKE_LIST    ( sizeof(makeList) / sizeof(SMakeList_T) )

I16 MakePktSubFront(CommHeader_T *comm, U8 *buf)
{
    U8 type = 0U;
